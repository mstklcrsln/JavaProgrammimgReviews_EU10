...
public class T6 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i <sentence.length() ; i++) {
           ...
           if (ch=='.') // we want to stop when compiler finds . dot.
              break; // if we not put it writes the other part of sentence (You are the best guysDo not care about it)

            if ( (ch>='a' && ch<='z') || (ch>='A' && ch<='Z') || ch==' ' )
                result+=ch;
        }
        System.out.println(result); //writes  You are the best guys
    }
}
